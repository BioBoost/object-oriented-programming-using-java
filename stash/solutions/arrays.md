...
## Freqtable of random numbers in range

```java
    Random generator = new Random();

    int[] freqTable = new int[13];

    for (int i = 0; i < 10000; i++) {

      int number = (int)(generator.nextDouble() * (22 - 10 + 1) + 10);
        // +1 because its 1.0 exclusive
    ...
    for (int i = 0; i < freqTable.length; i++ ) {
      System.out.println((i + 10) + ": " + freqTable[i]);
    }
